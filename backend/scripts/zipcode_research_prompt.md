# Zipcode Batch Research — Orchestrator Prompt

You are a real estate research agent. Analyze **{{LISTING_COUNT}}** rental listings in zipcode **{{ZIPCODE}}** ({{CITY}}, Canton {{CANTON}}) and produce a JSON research report for each one.
...
Three research agents investigate the **shared zipcode area** once (neighborhood, amenities with coordinates, market rents by room count). Then per-listing image analysis runs in parallel. You compile per-listing JSONs using each listing's actual lat/lng for accurate proximity distances.
...
> Research zipcode {{ZIPCODE}} in {{CITY}}, Canton {{CANTON}}.
>
> Use WebSearch for these queries (try both German and English):
> - "{{ZIPCODE}} {{CITY}} Quartier character neighborhood"
> - "{{ZIPCODE}} {{CITY}} Lärm noise levels Strasse"
> - "{{ZIPCODE}} {{CITY}} Sicherheit safety crime"
> - "{{ZIPCODE}} {{CITY}} Bauprojekte construction 2025 2026"
> - "{{ZIPCODE}} {{CITY}} Wohnqualität living quality"
>
> Return a structured text summary with these sections:
> - **neighborhood_character**: 1-2 sentence description of the area's vibe and character
...
> Find amenities in and near zipcode {{ZIPCODE}} {{CITY}}, Canton {{CANTON}}.
...
> Use queries like "[amenity type] {{ZIPCODE}} {{CITY}}" and "[amenity name] {{CITY}} address coordinates".
>
> Categories to research:
> - **public_transport**: Tram stops, bus stops, S-Bahn stations. Include line numbers.
>   Search: "Tramhaltestelle {{ZIPCODE}} {{CITY}}", "Bushaltestelle {{ZIPCODE}} {{CITY}}", "S-Bahn {{ZIPCODE}} {{CITY}}"
> - **supermarket**: Migros, Coop, Aldi, Lidl, Denner nearby.
>   Search: "Migros Coop {{ZIPCODE}} {{CITY}}", "Supermarkt {{ZIPCODE}} {{CITY}}"
> - **school**: Primary schools, secondary schools.
>   Search: "Schule {{ZIPCODE}} {{CITY}}", "Primarschule {{ZIPCODE}} {{CITY}}"
> - **park**: Parks and green spaces.
>   Search: "Park Grünanlage {{ZIPCODE}} {{CITY}}"
> - **medical**: Doctors, pharmacies, hospitals.
>   Search: "Arztpraxis Apotheke {{ZIPCODE}} {{CITY}}", "Spital {{ZIPCODE}} {{CITY}}"
> - **dining**: Restaurants, cafes.
>   Search: "Restaurant Café {{ZIPCODE}} {{CITY}}"
> - **fitness**: Gyms, sports facilities.
>   Search: "Fitnessstudio {{ZIPCODE}} {{CITY}}", "Sportanlage {{ZIPCODE}} {{CITY}}"
>
...
> Research the rental market for zipcode {{ZIPCODE}} {{CITY}}, Canton {{CANTON}}.
>
> Use WebSearch for:
> - "Mietpreise {{ZIPCODE}} {{CITY}} 2025 2026"
> - "average rent {{ZIPCODE}} {{CITY}} by room count"
> - "Wüest Partner Mietpreise {{CANTON}}"
> - "Homegate Mietpreisspiegel {{CITY}} {{ZIPCODE}}"
> - "comparis Miete {{ZIPCODE}} {{CITY}}"
> - "{{ZIPCODE}} {{CITY}} Durchschnittsmiete Zimmer"
>
...
```bash
cat /tmp/zipcode_{{ZIPCODE}}_area.json | python3 -c "import json,sys; print(json.dumps(json.load(sys.stdin)['amenities']))" | python3 backend/scripts/haversine.py <LISTING_LAT> <LISTING_LNG> > /tmp/listing_<ID>_distances.json
```
...
area = json.load(open('/tmp/zipcode_{{ZIPCODE}}_area.json'))
